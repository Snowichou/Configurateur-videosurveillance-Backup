...
/**
 * @param {{ getModel: () => object, getCatalog: () => object,
 *           getSteps: () => object[], getCameraById: (id:string)=>object,
 *           render: () => void }} deps
 */
...
// Appelle render (passé en deps) puis fire les after-render callbacks
...
function animateStepTransition() {
  ...
  if (!el) return;
  const idx = getModel()?.stepIndex ?? 0;
...
function injectCompareButtons() {
  document.querySelectorAll(".cameraPickCard").forEach(card => {
    ...
    const isIn = (getModel()?.ui?.compare || []).includes(camId);
    ...
    b.innerHTML = isIn ? "⚔️ " + T("cam_compare") + " ✓" : "⚔️ " + T("cam_compare");
...
function enhanceCompareCard() {
  ...
  const cmp = getModel()?.ui?.compare || [];
  if (cmp.length < 2) return;
  const getCam = getCameraById;
  const a = getCam(cmp[0]), b = getCam(cmp[1]);
  if (!a || !b) return;

  const esc = (v) => v == null ? "—" : String(v).replace(/[<>&"]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;'}[c]));
  const specs = [
    { l:T("cam_resolution"), k:"resolution_mp", u:"MP", i:"📐" },
    { l:"IR", k:"ir_range_m", u:"m", i:"🔦" },
    { l:"DORI Det.", k:"dori_detection_m", u:"m", i:"👁️" },
    { l:"DORI Id.", k:"dori_identification_m", u:"m", i:"🔍" },
    { l:"Focal min", k:"focal_min_mm", u:"mm", i:"🔭" },
    { l:"IP", k:"ip", u:"", i:"💧", p:"IP" },
  ...
  old.outerHTML = `
    <div class="enhancedCompare">
      <div class="cmpHeader">
        <div class="cmpTitle">${"⚔️ " + T("cam_compare")}</div>
...
function restoreFromURL() {
  try {
    ...
    const tryR = () => { if (getModel()) { applySnapshot(snap); callOriginalRender(); showToast("📂 Config restaurée depuis le lien", "success"); url.searchParams.delete("cfg"); window.history.replaceState({}, "", url.toString()); } else setTimeout(tryR, 200); };
    tryR();
  } catch {}
}
...
function validateCurrentStep() {
  ...
  const sid = (getSteps() || [])[m.stepIndex]?.id;
...
window.exportCatalogJSON = function () {
  const c = getCatalog();
  if (!c) { console.warn("Ajoute getCatalog() = CATALOG dans app.js"); return; }
...
function init() {
  _prevStepIndex = getModel()?.stepIndex ?? 0;
  createFloatingRecap();
  ...
  // Resize handler pour le swipe cards
